...
export function AdminProductEditor({ product }: { product: AdminProductView }) {
  ...
  return <article className="admin-product-editor">
    <header className="admin-product-heading"><div><div className="admin-product-title-row"><h2>{product.name}</h2><span className={`admin-status-badge is-${status}`}>{statusLabels[status]}</span></div><p>{product.description || '暂无商品说明'}</p><small>slug /{product.slug} · 最近更新 {new Date(product.updatedAt).toLocaleDateString('zh-CN')}</small></div><div className="admin-product-actions">{status !== 'published' && <button className="button button-primary" type="button" disabled={busy !== null} onClick={() => void setProductLifecycle('published')}>发布商品</button>}{status === 'published' && <button className="button button-danger" type="button" disabled={busy !== null} onClick={() => void setProductLifecycle('unlisted')}>归档商品</button>}</div></header>
...
export function AdminProductCreateForm() {
  const [form, setForm] = useState({ name: '', slug: '', description: '', skuName: '', skuSlug: '', price: '', status: 'draft' });
  const [message, setMessage] = useState('');
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch('/api/admin/products', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ name: form.name, slug: form.slug, description: form.description, skuName: form.skuName, skuSlug: form.skuSlug, priceCents: Math.round(Number(form.price) * 100), status: form.status, idempotencyKey: key() }) });
    ...
    setForm({ name: '', slug: '', description: '', skuName: '', skuSlug: '', price: '', status: 'draft' });
  }
  return <form className="admin-create-panel" onSubmit={submit}><div className="admin-create-heading"><div><span className="eyebrow">新增商品</span><h2>上新一个套餐</h2><p>先保存草稿，确认商品说明、价格和受理条件后再发布。</p></div><span className="admin-create-step">01 / 01</span></div><div className="admin-create-grid"><label>商品名称<input required value={form.name} onChange={(event) => update('name', event.target.value)} placeholder="例如 Claude Pro · 月卡" /></label><label>商品 slug<input required pattern="[a-z0-9]+(?:-[a-z0-9]+)*" value={form.slug} onChange={(event) => update('slug', event.target.value)} placeholder="claude-pro" /></label><label className="is-wide">商品说明<textarea required value={form.description} onChange={(event) => update('description', event.target.value)} placeholder="面向什么用户，如何交付，哪些情况不受理" /></label><label>套餐名称<input required value={form.skuName} onChange={(event) => update('skuName', event.target.value)} placeholder="Pro 月卡" /></label><label>套餐 slug<input required pattern="[a-z0-9]+(?:-[a-z0-9]+)*" value={form.skuSlug} onChange={(event) => update('skuSlug', event.target.value)} placeholder="claude-pro-monthly" /></label><label>价格（人民币）<input required min="0" step="0.01" inputMode="decimal" type="number" value={form.price} onChange={(event) => update('price', event.target.value)} placeholder="299" /></label><label>保存方式<select value={form.status} onChange={(event) => update('status', event.target.value)}><option value="draft">保存为草稿</option><option value="published">创建并立即上架</option></select></label></div><div className="admin-create-footer"><p>上架后公开站点会立即展示；归档只会停止新订单，不会删除历史订单。</p><button className="button button-primary" disabled={busy} type="submit">{busy ? '保存中…' : '创建商品'}</button></div>{message && <p className="admin-inline-message" role="status">{message}</p>}</form>;
}
