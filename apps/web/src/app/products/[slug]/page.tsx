...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!product) return { title: '商品不存在', robots: { index: false, follow: false } };
  const title = `${product.name}｜ChatGPT 会员充值`;
  const description = `${product.description} 了解适用账号、交付方式、保障规则与当前套餐价格。`;
  return {
    title,
    description,
    alternates: { canonical: canonicalUrl(`/products/${product.slug}`) },
    openGraph: {
      type: 'website', siteName, title, description,
      url: canonicalUrl(`/products/${product.slug}`),
      images: [{ url: siteOgImageUrl, width: 1200, height: 630, alt: `${product.name} · ChongHub` }],
    },
  };
}
...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const [product, settings, articles] = await Promise.all([
