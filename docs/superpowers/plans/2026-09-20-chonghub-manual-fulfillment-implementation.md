...
## 2026-09-21 执行补充（覆盖旧购买路径）

商品详情页本地检测 → 同页购买 → 订单生成动画 → 订单详情自动弹出微信；弹窗支持关闭和再次打开。游客免查询密码，使用邮箱验证码找回订单。实现必须覆盖重复点击、失败重试、游客再次购买、订单创建成功但检测保存失败的恢复路径；测试只使用合成会话数据。在线支付仍不在本轮范围内。
