...
## 2026-09-21 购买路径修订（优先于下文旧流程）

- 商品详情页直接提供账号检测，不再要求先提交需求、跳转后才能检测。面向用户使用“检测”，内部 screening 字段不变。
- 账号检测通过后，用户在同页选择套餐、填写联系邮箱并点击购买；游客不设置查询密码。跨设备查单使用订单号、联系邮箱及邮箱验证码。
- 生成订单期间显示真实请求进度的加载动画，不模拟付款成功；首版仍不接在线支付。
- 创建成功进入订单详情页，并自动弹出后台配置的微信二维码。可主动关闭，订单页“查看微信交付说明”可再次打开。
- 检测仅解析浏览器内会话快照，不代表实时充值资格或履约授权；只提交精简结果，禁止上传或持久化原文。
- 订单已创建而检测结果保存失败时，应显示需重新检测的明确提示，不提示订单未生成、不自动创建第二张订单。
