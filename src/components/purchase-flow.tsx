...
import { classifySession } from '@/modules/screening/classify.client';
import type { ScreeningReport } from '@/modules/screening/report-schema';
  ...
  return <section className="purchase-flow" id="purchase">
    ...
    <div className="purchase-grid">
      ...
      <div className="form-card purchase-order-card">
        <label>选择套餐<select value={skuId} onChange={(event) => setSkuId(event.target.value)}>{product.skus.map((sku) => <option key={sku.id} value={sku.id}>{sku.name} · ¥{(sku.priceCents / 100).toFixed(0)}</option>)}</select></label>
        ...
        <button className="button button-primary" type="button" disabled={busy} onClick={() => void buy()}>{busy ? '正在生成订单…' : '立即购买并生成订单'}</button>
