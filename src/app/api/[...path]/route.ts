...
async function handler(request: Request, { params }: Ctx) {
  ...
  if (request.method === 'PATCH' && productAdminMatch) { if (!actor || actor.kind !== 'admin') throw new AppError('FORBIDDEN', '无权访问后台。', 403); const data=await body(request); await updateCatalogEntry(productAdminMatch[1], { name: typeof data.name==='string'?data.name:undefined, description: typeof data.description==='string'?data.description:undefined, priceCents: typeof data.priceCents==='number'?data.priceCents:undefined, skuId: typeof data.skuId==='string'?data.skuId:undefined }); return jsonData({ updated:true }); }
