...
export default function LoginPage() {
  ...
  async function send() { setBusy(true); setMessage(''); const response=await fetch('/api/auth/otp',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({email,purpose:'login'})}); const json=await response.json(); setBusy(false); if(!response.ok){setMessage(json.error?.message??'发送失败');return;} setSent(true); setMessage('验证码已发送，请查收邮箱。'); }
  async function submit(event:FormEvent) { event.preventDefault(); setBusy(true); const response=await fetch('/api/auth/verify',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({email,code})}); const json=await response.json(); setBusy(false); if(!response.ok){setMessage(json.error?.message??'验证失败');return;} router.push('/me'); }
