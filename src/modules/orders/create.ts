...
import { hmacHex, randomToken, sha256Hex } from '@/server/crypto';
...
export async function createRequest(input: CreateRequestInput, actor: Actor | null, key: string, draftScope: string): Promise<{ order: PublicOrder; guestToken?: string }> {
  ...
  return withTransaction(async (client) => {
    ...
    const sku = await client.query<{ id: string; product_name: string; product_eligibility: string; product_screening: 'gpt_session' | 'none'; sku_name: string; cycle_text: string; price_cents: number; warranty_text: string }>(
      `SELECT s.id, p.name product_name, p.eligibility_text product_eligibility, p.screening_method product_screening,
              s.name sku_name, s.cycle_text, s.price_cents, s.warranty_text
       FROM skus s JOIN products p ON p.id=s.product_id
       WHERE s.id=$1 AND s.status='published' AND p.status='published' FOR SHARE`, [normalized.skuId],
    );
    const product = sku.rows[0];
    if (!product) throw new AppError('SKU_UNAVAILABLE', '该套餐暂不可提交，请刷新后重试。', 409);
